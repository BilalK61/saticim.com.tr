...
const Modal = ({
    ...
    cancelText = 'İptal',
    showCancel = false,
    children,           // Özel içerik için
    customIcon,         // Özel ikon için (React element)
    size = 'sm',        // 'sm' | 'md' | 'lg'
    hideDefaultButtons = false,  // Varsayılan butonları gizle
    confirmDisabled = false,     // Onay butonu devre dışı
    confirmLoading = false       // Onay butonu yükleniyor
}) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
            <div
                className={`bg-white rounded-3xl shadow-2xl w-full ${sizeClasses[size]} overflow-hidden transform transition-all scale-100 relative`}
                    ...
                    {!hideDefaultButtons && (
                        <div className="flex gap-3 justify-center">
                            ...
                            <button
                                ...
                                className={`flex-1 px-6 py-3 rounded-2xl text-white font-semibold shadow-lg transition duration-200 hover:-translate-y-0.5 ${buttonColors[type]} disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0`}
                            >
                                {confirmLoading ? (
